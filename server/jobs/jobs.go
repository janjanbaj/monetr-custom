...
func RegisterJobs(
	...
	// Only include lunch flow jobs if its actually enabled.
	if config.LunchFlow.Enabled {
	...
	// Same with billing/billing related jobs.
	if config.Stripe.IsBillingEnabled() {
		if err := myownsanity.FirstError(
	...
	return myownsanity.FirstError(
		...
		queue.RegisterCron(ctx, processor, plaid_jobs.SyncPlaidAccountsCron, "0 15 */12 * * *"),
