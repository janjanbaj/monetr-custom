...
func (c *Controller) postFixedDeposit(ctx echo.Context) error {
	...
	if req.InterestFrequency != "monthly" && req.InterestFrequency != "quarterly" && req.InterestFrequency != "end_of_term" {
		return c.badRequest(ctx, "interest frequency must be monthly, quarterly, or end_of_term")
	}
	if req.InterestDestination != "accumulate" && req.InterestDestination != "payout" {
		return c.badRequest(ctx, "interest destination must be accumulate or payout")
	}
	if req.InterestDestination == "payout" && (req.InterestDestinationAccountId == nil || req.InterestDestinationAccountId.IsZero()) {
		return c.badRequest(ctx, "must specify a valid interest destination account ID for payout")
	}
	...
	if sourceAccount.Status == BankAccountStatusInactive {
		return c.badRequest(ctx, "source account is inactive")
	}
	...
	if err := repo.UpdateBankAccount(c.getContext(ctx), sourceAccount); err != nil {
		return c.wrapPgError(ctx, err, "failed to update source bank account balance")
	}

	cdAccount.AvailableBalance = req.Amount
	cdAccount.CurrentBalance = req.Amount
	if err := repo.UpdateBankAccount(c.getContext(ctx), &cdAccount); err != nil {
		return c.wrapPgError(ctx, err, "failed to update CD bank account balance")
	}

	// 4. Create interest payout FundingSchedule if rate > 0
	var scheduleId *ID[FundingSchedule]
	if req.InterestRate > 0 {
		interestDestId := cdAccount.BankAccountId // MUST be cdAccount.BankAccountId due to DB foreign key constraint
		...
		if interestAmount > 0 {
			...
			if err != nil {
				return c.badRequest(ctx, "failed to parse rule: %v", err)
			}
			...
			if err := repo.CreateFundingSchedule(c.getContext(ctx), &interestSchedule); err != nil {
				return c.wrapPgError(ctx, err, "failed to create interest funding schedule")
			}
	...
	fd := FixedDeposit{
		...
		InterestDestinationBankAccountId: req.InterestDestinationAccountId,
...
func (c *Controller) postFixedDepositWithdraw(ctx echo.Context) error {
	...
	fixedAccount, err := repo.GetBankAccount(c.getContext(ctx), fd.BankAccountId)
	if err != nil {
	...
	if err := repo.CreateTransaction(c.getContext(ctx), fd.BankAccountId, &txnDebit); err != nil {
	...
	if err != nil {
		return c.wrapPgError(ctx, err, "failed to get source bank account")
	}
	...
	if err := repo.UpdateBankAccount(c.getContext(ctx), fixedAccount); err != nil {
		return c.wrapPgError(ctx, err, "failed to update CD bank account status")
	}
	...
	if fd.FundingScheduleId != nil {
		...
		if err := repo.DeleteFundingSchedule(c.getContext(ctx), fd.BankAccountId, scheduleId); err != nil {
			return c.wrapPgError(ctx, err, "failed to delete interest funding schedule")
		}
	} else {
		fd.Status = FixedDepositStatusWithdrawn
		if err := repo.UpdateFixedDeposit(c.getContext(ctx), fd); err != nil {
