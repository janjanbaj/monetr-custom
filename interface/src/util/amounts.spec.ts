...
describe('amounts', () => {
  ...
  describe('format amount', () => {
    ...
    it('will format NPR properly', () => {
      const foo = formatAmount(1234, AmountType.Stored, 'en-US', 'NPR');
      expect(foo).toBe('NPR 12.34');

      const bar = formatAmount(12.34, AmountType.Friendly, 'en-US', 'NPR');
      expect(bar).toBe('NPR 12.34');
    });
  });
});
